...
async def async_setup_entry(
    ...
    last_seen_entities = [LuminaryMotionLastSeenSensor(coordinator, sid) for sid in coordinator.sensors]
    coordinator.last_seen_entities = {
        entity.monitored_sensor: entity for entity in last_seen_entities
    }

    async_add_entities([status_entity, *hw_timeout_entities, *last_seen_entities])
...
class LuminaryMotionLastSeenSensor(SensorEntity):
    """Read-only display of one configured motion sensor's last-communicated timestamp.

    Exposed as device_class=timestamp so the frontend renders it as a live "X minutes
    ago" — the same signal coordinator._check_stale_sensors() polls to notify when a
    sensor goes quiet for too long (see const.py comment). Value is never cached here —
    see coordinator.sensor_last_seen() — the owning integration owns this value.
    """
    ...
    def __init__(self, coordinator: ZoneCoordinator, sensor_entity_id: str) -> None:
        ...
        state = coordinator.hass.states.get(sensor_entity_id)
        friendly = state.attributes.get("friendly_name") if state else sensor_entity_id
        self._attr_name = f"{friendly} Last Seen"
        self._attr_unique_id = f"{coordinator.entry.entry_id}_last_seen_{slugify(sensor_entity_id)}"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, coordinator.entry.entry_id)},
            name=coordinator.zone_name,
            manufacturer="Luminary HA",
    ...
    def native_value(self):
        return self._coordinator.sensor_last_seen(self.monitored_sensor)
